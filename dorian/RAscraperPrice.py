...
def get_event_data(event_url):
	...
	soup = BeautifulSoup(raw_page)
	return tickets_available(soup), event_cost(soup), event_date_city(soup), lineup(soup), promoter(soup), venue(soup), members_attending(get_event_number(event_url))
...
for i in range(0,len(df_urls)):
	...
	print get_event_data(ev_url), ev_url
