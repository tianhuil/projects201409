...
def AddClassifiers(tripdata, classifiers):
    for c in classifiers:
        ...
        if c['t'] == "Round":
            tripdata[c['colname']] = map(lambda s, e: IsRoundTrip(s,e), 
...
def MakeBlockGroupData(extra_cols):
    BlockGroupsNearStations = []
    for i in range(len(sr)):
        rec = sr[i]
        if i%100 == 0:
            print str(i) + '/' + str(len(sr))
        if BlockGroupNearAStation(rec, station_census):
...
BlockGroupsNearStations = MakeBlockGroupData([{'f':"Data/AgeBySex/AgeBySex.csv",'cols':[{'in_f':'Population','in_df':'D001'}]},
